...
from project2.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRET_KEY
import razorpay
...
def confirmationPage(Request):
    ...
    if(username):
        try:
            ...
            message = "Thanks to Shop with Us!!! Your Order Has Been Placed!!! Now You Can Track Your Order in Profile Section"
...
def shopPage(Request, mc, sc, br):
    if (mc == "All" and sc == "All" and br == "All"):
        ...
        data = Product.objects.filter(brand=Brand.objects.get(
            name=br), subcategory=Subcategory.objects.get(name=sc))
    else:
        data = Product.objects.filter(maincategory=Maincategory.objects.get(
            name=mc), brand=Brand.objects.get(name=br), subcategory=Subcategory.objects.get(name=sc))
    count = len(data)
    data = data[::-1]
    maincategory = Maincategory.objects.all()
    subcategory = Subcategory.objects.all()
...
def sortFilterPage(Request, mc, sc, br):
    if (Request.method == "POST"):
        ...
        if (mc == "All" and sc == "All" and br == "All"):
            ...
            data = Product.objects.filter(
                brand=Brand.objects.get(name=br)).order_by(sort)
        elif (mc != "All" and sc != "All" and br == "All"):
            data = Product.objects.filter(maincategory=Maincategory.objects.get(
                name=mc), subcategory=Subcategory.objects.get(name=sc)).order_by(sort)
        elif (mc != "All" and sc == "All" and br != "All"):
            data = Product.objects.filter(maincategory=Maincategory.objects.get(
                name=mc), brand=Brand.objects.get(name=br)).order_by(sort)
        elif (mc == "All" and sc != "All" and br != "All"):
            data = Product.objects.filter(brand=Brand.objects.get(
                name=br), subcategory=Subcategory.objects.get(name=sc)).order_by(sort)
        else:
...
def searchPage(Request):
    if (Request.method == "POST"):
        ...
        data = Product.objects.filter(Q(name__icontains=search) | Q(color__icontains=search) | Q(
            size__icontains=search) | Q(description__icontains=search))
        maincategory = Maincategory.objects.all()
        subcategory = Subcategory.objects.all()
        brand = Brand.objects.all()
        count = len(data)
    ...
    if (Request.method == "POST"):
        ...
        try:
            ...
            if (user.is_superuser):
                ...
                recipient_list = [buyer.email,]
                send_mail(subject, message, email_from, recipient_list)
                return HttpResponseRedirect("/forgot-2/")
        except:
            messages.error(Request, "Invalid Username")
    brand = Brand.objects.all()
    subcategory = Subcategory.objects.all()
    maincategory = Maincategory.objects.all()
    return render(Request, "forgot-1.html",{'maincategory':maincategory,'subcategory':subcategory,'brand':brand})
...
def forgotPasswordPage2(Request):
    ...
    if (Request.method == "POST" and username):
        ...
        try:
            ...
            if (buyer.otp == int(otp)):
                ...
                return HttpResponseRedirect("/forgot-3/")
            else:
    ...
    subcategory = Subcategory.objects.all()
    maincategory = Maincategory.objects.all()
    return render(Request, "forgot-2.html",{'maincategory':maincategory,'subcategory':subcategory,'brand':brand})
    ...
    if(otp):
        if (Request.method == "POST"):
            ...
            if (resetUser and otp):
                ...
                subcategory = Subcategory.objects.all()
                maincategory = Maincategory.objects.all()
                return render(Request, "forgot-3.html",{'maincategory':maincategory,'subcategory':subcategory,'brand':brand})
    else:
        return HttpResponseRedirect("/forgot-1/")
